...
You can hide folders and single elements from code, while still receiving their values in code - the only change is visual. This is helpful when you have a list of autogenerated folders whose paths differ only by the 'i' in a loop, and you create 10 folders and then decrease the limit - you want to extra folders to go away to not clutter up your screen when unused.

- `gui.hide("myPath")` hide anything at this path (the prefix stack applies here like everywhere else)
- `gui.show("myPath")` show anything at this path that was previously hidden
...
Subsequent calls of the same method and path have no effect, these methods are 'idempotent'. 
In essence: once it's already hidden, any additional `hide()` call is safely ignored.
